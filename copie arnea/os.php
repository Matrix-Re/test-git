...
if (!empty($_POST['lien']) && !empty($_POST['page']) && !empty($_POST['google']) && !empty($_POST['https'])) {   
   $complet = implode(';', $_POST['lien']);
   $complet2 = implode(';', $_POST['page']);
   $complet3 = implode(';', $_POST['https']);
   $complet4 = implode(';', $_POST['google']);
   header('location:phasedepayement2.php');
   $req = $db->prepare("INSERT INTO os(lien, page, google, https) VALUES(?, ?, ?, ?)");
   $req->execute(array($complet, $complet2, $complet3, $complet4));
